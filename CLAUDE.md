...
- Landing: `/welcome` is the campaign landing page (ads, social, and the board's "How it works" link): light, photo-led, built on live data — a real market card in the hero, live topic counts, trending markets. It renders full-bleed (AppShell `isMarketingRoute`), and its money-word legal lines are inline English constants in `components/welcome/WelcomeSections.tsx`.
